...
% 가중치 레이블 (일부만 표시) — 마지막 점은 아래쪽으로 오프셋
label_idx  = [1,   5,   10,  15,  19  ];
offsets_x  = [200, 200, 200, 200, 200 ];
offsets_y  = [0,   0,   0,   0,  -0.0004];

for i = 1:length(label_idx)
    k = label_idx(i);
    text(V_sweep(k) + offsets_x(i), eta_sweep(k) + offsets_y(i), ...
        sprintf('w_V=%.2f', wV_vec(k)), 'FontSize', 8, 'Color', 'k');
end

% 단목적 및 가중합 기준점 표시 (초기 설계 제외)
scatter(V_single, eta_single, 100, 'r', '^', 'filled', 'DisplayName','단목적(체적)');
scatter(V_opt,    eta_opt,    100, 'b', 'p', 'filled', 'DisplayName','가중합 w_V=0.5');

% 축 범위를 스윕 결과 기준으로 설정
x_margin = (max(V_sweep) - min(V_sweep)) * 0.1;
y_margin = (max(eta_sweep) - min(eta_sweep)) * 0.15;
xlim([min(V_sweep) - x_margin, max(V_sweep) + x_margin]);
ylim([min(eta_sweep) - y_margin, max(eta_sweep) + y_margin]);

legend('Location','best');
xlabel('체적 V (mm³)');
ylabel('전달 효율 η');
title('Figure 5: 체적-효율 트레이드오프 곡선 (가중치 스윕, SQP)');
grid on;
hold off;
