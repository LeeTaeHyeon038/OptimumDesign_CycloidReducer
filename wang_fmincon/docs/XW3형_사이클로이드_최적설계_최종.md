...
스윕 결과를 체적-효율 평면에 그리면 단조 감소하는 곡선이 나타난다. 체적이 작아질수록 효율도 낮아지는 방향으로 움직이므로, 두 목표 사이에 **완만한 상충 관계**가 존재한다. 그러나 그 기울기가 완만해서 체적을 약 8,200 mm³(15%) 줄이는 대가로 효율이 약 0.006(0.6%p)만 감소한다.

또한 결과가 연속적으로 변하지 않고 **계단식으로 클러스터를 형성**하는 것이 관찰된다. 이는 SQP가 비선형 다목적 문제에서 가중치 변화에 따라 서로 다른 국소해로 수렴하는 현상이다. 전역 최적해를 보장하지 않는 SQP의 특성이 반영된 결과이며, 이 점에서 가중합법 + SQP는 근사 Pareto 프론트를 탐색하는 방법으로 이해할 수 있다.
...
3. Arora, J. S. (2016). *Introduction to Optimum Design* (4th ed.). Elsevier Academic Press.: 수치적 체적 계산 방식 적용 시도 및 한계
