...
$$\min_{d_{rp},\ K_1} \quad V(d_{rp},\ K_1)$$

$$\text{s.t.} \quad g_i(d_{rp},\ K_1) \leq 0, \quad i = 1, 4, 5$$

$$7.0 \leq d_{rp} \leq 10.4\ \text{mm}, \quad 0.60 \leq K_1 \leq 0.90$$

나머지 13개 제약($D_p$, $B$, $D$, $d_{sw}$ 관련)은 고정값 대입 시 상수가 되어 항상 만족되므로 생략한다. 실질적으로 활성되는 비선형 제약은 다음 3개이다.

**y1 — 언더컷/첨예화 방지** ($K_1 \geq 0.60$이므로 항상 첫 번째 분기 적용):

$$g_1 = \frac{d_{rp}}{2} - \frac{D_p}{2}\sqrt{\frac{27(1-K_1^2)(z_p-1)}{(z_p+1)^3}} \leq 0$$

**y4 — 핀직경 계수 하한** ($K_2 \geq 1.0$):

$$g_4 = 1.0 - \frac{D_p}{d_{rp}}\sin\frac{\pi}{z_p} \leq 0$$

**y5 — 핀직경 계수 상한** ($K_2 \leq 1.6$):

$$g_5 = \frac{D_p}{d_{rp}}\sin\frac{\pi}{z_p} - 1.6 \leq 0$$

y5의 경계는 $d_{rp} = D_p \sin(\pi/z_p)/1.6 = 6.24\ \text{mm}$에 해당하여 설계변수 하한(7.0 mm)보다 작으므로, $d_{rp} \geq 7.0$ 범위 내에서는 y5가 항상 만족된다. 즉 **2변수 문제에서 실질적으로 최적해를 제한하는 활성 제약은 y1과 y4** 두 개이다.
...
| 항목 | 설정값 |
|---|---|
| 목표 설정 | 체적 수식 셀 $V(d_{rp}, K_1)$ (최솟값) |
| 변수 셀 | $d_{rp}$, $K_1$ (2개) |
| 해법 | GRG 비선형 (GRG Nonlinear) |
| 제약조건 | $7.0 \leq d_{rp} \leq 10.4$, $0.60 \leq K_1 \leq 0.90$, $g_1 \leq 0$, $g_4 \leq 0$, $g_5 \leq 0$ |
...
10절의 2변수 축소 문제와 동일한 설정에서, 수업에서 학습한 **도식해법(Graphical Method)**을 적용하여 최적해를 시각적으로 확인한다. 도식해법은 설계변수가 2개인 문제에서 제약조건 경계선과 목적함수 등축선을 같은 평면에 그려 가용 영역과 최적해의 위치를 직접 파악하는 방법이다.

**2변수 축소 문제 정식화:**

$$\min_{d_{rp},\ K_1} \quad V(d_{rp},\ K_1)$$

$$\text{s.t.} \quad g_1 = \frac{d_{rp}}{2} - \frac{D_p}{2}\sqrt{\frac{27(1-K_1^2)(z_p-1)}{(z_p+1)^3}} \leq 0 \quad \text{(언더컷 방지)}$$

$$g_4 = 1.0 - \frac{D_p}{d_{rp}}\sin\frac{\pi}{z_p} \leq 0 \quad (K_2 \geq 1.0)$$

$$g_5 = \frac{D_p}{d_{rp}}\sin\frac{\pi}{z_p} - 1.6 \leq 0 \quad (K_2 \leq 1.6)$$

$$7.0 \leq d_{rp} \leq 10.4\ \text{mm}, \quad 0.60 \leq K_1 \leq 0.90$$

$d_{rp}$를 x축, $K_1$을 y축으로 하는 2차원 평면에 각 제약조건 경계선($g_i = 0$)과 체적 등축선을 그려 가용 영역 전체를 시각화하였다. MATLAB의 `meshgrid`와 `contour` 함수로 구현하였다.
...
| $g_1 = 0$ | 검은 실선 | 언더컷/첨예화 방지 경계 — 우상단이 위반 영역 |
| $g_4 = 0$ | 파란 수직선 | $K_2 = 1.0$ 경계 ($d_{rp} = 9.99$ mm) — 우측이 위반 |
| $g_5 = 0$ | 자홍 수직선 | $K_2 = 1.6$ 경계 ($d_{rp} = 6.24$ mm) — 좌측이 위반 |
| 빨간 실선 | 수직/수평 | 설계변수 상·하한 경계 ($d_{rp}$, $K_1$ 범위) |
| 검은 점선 | — | 체적 등축선 $V = $ const |
| 빨간 원 | — | 최적해 $\mathbf{X}^* = (9.990,\ 0.775)$, $V^* = 55,125\ 	ext{mm}^3$ |
| 파란 사각형 | — | 초기점 $\mathbf{X}_0 = (10.0,\ 0.6069)$, $V_0 = 56,651\ 	ext{mm}^3$ |
| 초록 화살표 | — | 체적 감소 방향 (등축선이 작아지는 방향, 우상단) |

### 11.3 도식해법으로 확인한 인사이트

**가용 영역의 형태**: 가용 영역은 $g_1 = 0$ 곡선(좌상단 경계), $g_4 = 0$ 수직선(우측 경계), 설계변수 하한 $d_{rp} = 7.0$ mm(좌측), $K_1 = 0.60$(하단), $K_1 = 0.90$(상단)으로 둘러싸인 영역이다. $g_5 = 0$ 경계($d_{rp} = 6.24$ mm)는 설계변수 하한 밖에 위치하므로 가용 영역 내에서는 y5가 항상 만족된다.

**최적해의 위치**: 최적점 $\mathbf{X}^*$은 $g_1 = 0$ 경계(언더컷 방지)와 $g_4 = 0$ 경계($K_2$ 하한)의 **교점**에 위치한다. 이는 6절 KKT 분석에서 y1과 y4가 활성 제약으로 확인된 결과와 정확히 일치하며, 두 제약이 동시에 최적해를 제한하고 있음을 시각적으로 직접 확인할 수 있다.

**체적 감소 방향**: 등축선을 체적이 작아지는 방향(우상단)으로 이동하면 $g_1 = 0$과 $g_4 = 0$의 교점에서 멈추게 된다. 더 이상 체적을 줄이려면 두 경계 중 하나를 넘어야 하므로 이 교점이 현재 설계 조건에서의 한계점이다.

**초기점에서 최적해로의 이동**: 초기점 $\mathbf{X}_0 = (10.0,\ 0.6069)$는 가용 영역 내부에 있다. 최적화는 $d_{rp}$를 거의 유지하면서 $K_1$을 0.6069에서 0.775로 상승시켜 $g_1 = 0$ 경계에 도달하는 방향으로 진행되었다.
